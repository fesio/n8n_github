...
- **Automatyzacja all-in**
  ...
  - uruchom lokalnego agenta `AGENT_INTERVAL=3600 python scripts/strategy_agent.py` jako usługę (systemd/pm2), gdzie `AGENT_INTERVAL=3600` oznacza interwał 1h
...
- **self-host (docker, 2 minuty)**
  ...
  - po starcie: skonfiguruj URL + API token, podmień `N8N_BASE_URL` w `.env` (plik `.env` w katalogu głównym, kopia z `.env.example`)
